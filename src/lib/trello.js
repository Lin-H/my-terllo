...
wrapper = function(window, opts) {
  ...
  Trello = {
    ...
    rest: function() {
      ...
      if (params != null) {
        Object.assign(opts.data, params);
      }
      return $.ajax(opts);
    },
    ...
    addCard: function(options, next) {
      ...
      getCard = function(callback) {
        ...
        return window.open(intentEndpoint + "/add-card?" + $.param(Object.assign(baseArgs, options)), "trello", "width=" + width + ",height=" + height + ",left=" + left + ",top=" + top);
      };
      if (next != null) {
        return getCard(next);
  ...
  authorizeURL = function(args) {
    ...
    return authEndpoint + "/" + version + "/authorize?" + $.param(Object.assign(baseArgs, args));
  };
